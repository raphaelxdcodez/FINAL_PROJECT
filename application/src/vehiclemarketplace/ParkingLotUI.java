	...
	public ParkingLotUI(ParkingSpotListener listener, boolean[][] savedData) {
		...
		DefaultTableModel model = new DefaultTableModel(8, 10) {
			...
			public boolean isCellEditable(int row, int column) {
				return false; // Disable typing in cells
			}
		};

		// JTable
		JTable table = new JTable(model);
		table.setRowHeight(70); // Taller rows look more like parking stalls
		table.setIntercellSpacing(new Dimension(10, 10));
		table.setGridColor(Color.YELLOW); // Yellow lines like a real lot
		table.setBackground(Color.LIGHT_GRAY);
		table.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		table.setTableHeader(null); // Remove headers for a cleaner look
		...
		table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table, Object value,
				...
				if ("OCCUPIED".equals(value)) {
					...
					c.setBackground(new Color(60, 60, 60)); // Dark Gray
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (row != -1 && col != -1) {
					...
					if (savedData[row][col]) {
						JOptionPane.showMessageDialog(null, "Spot already taken!");
						return;
					}
		...
		for (int r = 0; r < 8; r++) {
			for (int c = 0; c < 10; c++) {
				if (this.savedData[r][c]) { // Check the saved array
					table.setValueAt("OCCUPIED", r, c); // Mark it in the table
				}
			}
		}
